...
(function() {
  ...
  HiveToon.prototype.fetchSeriesPayload = async function(seriesId) {
    if (isObject(this.cachedSeriesPayloads[seriesId])) {
      return this.cachedSeriesPayloads[seriesId];
    }

    var html = await this.fetchText(this.getMangaShareUrl(seriesId));
    var payload = extractFlightObject(html, "{\"post\":", "series");
    var seriesPayload = isObject(payload) ? payload.post : null;
    if (!isObject(seriesPayload)) {
      throw new Error("Unable to decode the HiveToon series payload for " + seriesId + ".");
    }
    this.cachedSeriesPayloads[seriesId] = seriesPayload;
    return this.cachedSeriesPayloads[seriesId];
  };
  ...
  // HiveToon's server-rendered pages ship the canonical series/chapter payloads
  // inside the decoded RSC flight stream. Reuse that instead of scraping large
  // rendered card trees or guessing undocumented private API endpoints.
